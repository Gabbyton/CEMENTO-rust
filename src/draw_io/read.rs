pub use diagram_read::read_diagram;

pub mod diagram_read {
    use crate::draw_io::types::{DiagramEdge, DiagramElement, DiagramTerm};
    use ammonia::Builder;
    ...
    fn parse_drawio_file(xml_content: &str) -> Vec<DiagramElement> {
        ...
        for element in diagram_root.children().filter(|child| child.is_element()) {
            ...
            let default_attributes: HashSet<&str> =
                HashSet::from(["id", "value", "parent", "style"]);
    ...
    pub fn read_diagram(file_path: &str) -> DiGraph<DiagramTerm, DiagramEdge> {
        let file_path = Path::new(file_path);
        let xml_content = std::fs::read_to_string(file_path).expect("cannot read drawio diagram");
        ...
        let mut diagram_graph = DiGraph::<DiagramTerm, DiagramEdge>::new();
        for edge in graph_edges {
            let (source_term, target_term, graph_edge) = edge;
            let source_idx = diagram_graph.add_node(source_term);
            let target_idx = diagram_graph.add_node(target_term);
            diagram_graph.add_edge(source_idx, target_idx, graph_edge);
        }
        diagram_graph
    }
}
